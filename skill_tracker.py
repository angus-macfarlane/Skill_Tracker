...
def add_to_frame(entry_text, time, coord=None):
    ...
    new_label.grid(row=row, column=column, padx=5, pady=10)

    column_counter += 1
    skill_dictionary.setdefault(entry_text, []).append(column)
    skill_dictionary[entry_text].append(row)
    if column_counter == 3:
        column_counter = 0
        row_counter += 1
...
def add_to_frame_button_command():
...
if __name__ == '__main__':
    ...
    submit_button = tk.Button(main_window, text="Submit", command=add_to_frame_button_command)
